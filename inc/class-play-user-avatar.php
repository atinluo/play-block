...
class Play_User_Avatar {
    ...
    public function save_avatar( $user_id, $file = NULL ) {
        ...
        $max_avatar_img_size = apply_filters('play_max_avatar_img_size', 1024000);
        if($file[ 'avatar' ][ "size" ] > $max_avatar_img_size){
            wp_die( sprintf( __( 'Maximum upload image size: %s.', 'play-block' ), esc_html( size_format( $max_avatar_img_size ) ) ) );
            return;
        }
