...
class Play_Playlist {
    ...
    public function __construct() {
        ...
        add_action( 'the_playlist_button', array( $this, 'the_playlist_button' ), 10, 2 );
    }
     ...
     * playlist button
     */
    public function the_playlist_button( $post_id, $class="" ) {
        $type = get_post_meta($post_id, 'type', true);
        if($type == 'single'){
            echo $this->get_playlist_button($post_id, $class);
        }
    }

    public function get_playlist_button( $post_id, $class ) {
        $button = '<button data-id="%s" class="btn-playlist %s"><span>'.play_get_text('add-to-playlist').'</span></button>';
        $ret = sprintf( $button, $post_id, $class );
        return apply_filters( 'play_get_playlist_button', $ret, $post_id, $button, $this );
    }
